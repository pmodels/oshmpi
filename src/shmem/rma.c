...
{
    OSHMPI_ctx_put(ctx, MPI_BYTE, source, dest, nelems, pe);
}

void shmem_putmem(void *dest, const void *source, size_t nelems, int pe)
{
    OSHMPI_ctx_put(SHMEM_CTX_DEFAULT, MPI_BYTE, source, dest, nelems, pe);
}

void shmem_ctx_getmem(shmem_ctx_t ctx, void *dest, const void *source, size_t nelems, int pe)
{
    OSHMPI_ctx_get(ctx, MPI_BYTE, dest, source, nelems, pe);
}
